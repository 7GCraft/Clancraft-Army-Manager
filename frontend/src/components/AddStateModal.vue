              ...
              <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                ></font-awesome-icon>
                  ...
                  <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div class="mt-2">
                      <form @submit="$emit('submitModal',stateData)" id="add-state-form">
                            <div class="form-control mt-1">
                                <label for="state-name" class="mr-[52px]">State Name</label>
                                <input v-model="stateData.name" type="text" class="border border-black p-1"/>
                            </div>
                            <div class="form-control mt-1">
                                <label for="state-name" class="mr-20">State ID</label>
                                <input v-model="stateData.id" type="text" class="border border-black p-1"/>
                            </div>
                            <div class="form-control mt-1">
                                <label for="state-name" class="mr-8">State Currency</label>
                                <input v-model="stateData.currency" type="text" class="border-black border p-1"/>
                            </div>
                            <div class="form-control mt-1">
                                <label for="state-name" class="mr-[64px]">State Map</label>
                                <select v-model="stateData.map" class="border border-black p-1 w-48">
                                <option :key="key" v-for="(value,key) in filteredStateMap" :value="value">
                              {{ value }}
                                </option>
                                </select>
                            </div>
